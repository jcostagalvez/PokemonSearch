...
const inizialice = () =>{
    return new Promise((resolve, reject) => {
        ...
        let request = indexdb.open("pokemonDataBase", 1);
        ...
        request.onupgradeneeded = (event) => {
            const database = event.target.result;
            const objectStore = database.createObjectStore('myStore', {keyPath : 'url', autoIncrement: false});
            objectStore.createIndex ('url_names', 'url', {unique: true}),
            resolve(database);
        };
    })
};
...
const openTransaction = (database, mode) => {
    const transaction = database.transaction('myStore', mode);
    return transaction.objectStore('myStore');
};
...
const cacheResponse = (json) => { return { json: () => json } };
